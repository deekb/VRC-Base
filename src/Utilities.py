...
class TrapezoidalMovementProfile:
    ...
    def check_end_condition_met(self):
        return self.distance_travelled >= self.target_travel_distance

    def update(self, distance_remaining):
        ...
        if self.state == self.START:
            ...
            distance_that_will_be_traveled_during_deceleration = (
                (self.current_target_speed**2) * self.deceleration_time
            ) / (2 * self.target_travel_speed)

            distance_remaining_after_deceleration = (
                distance_remaining - distance_that_will_be_traveled_during_deceleration
            )
        ...
        if self.state == self.ACCELERATING:
            ...
            self.current_target_speed = (
                self.target_travel_speed / self.acceleration_time
            ) * (self.timer.time(SECONDS) - self.acceleration_start_time)
